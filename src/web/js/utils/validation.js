...
Validation.prototype = {
	//initialize : function(form,inPlaceEditor,options){
		...
		if(Validation.isVisible(this) && !v.test($F(this))) {
			if(!this[prop]) {
				...
				if(typeof advice == 'undefined') {
					...
					//advice.setAttribute('width', '100');
					advice.style.width= '100px';
					//advice.style.display = 'none';
					//advice.style.zIndex= 500;
					//this.parentNode.insertBefore(advice, this.nextSibling);*/
					alert(v.error);
					//advice.style.width= '100px';
					//advice.setAttribute('style','display:table-cell;float:left;background-color:blue;');
					//this.parentNode.parentNode.insertBefore(advice, this.parentNode.nextSibling);
				}
			/*	if(typeof Effect == 'undefined') {
					advice.style.display = 'block';
				} else {
					new Effect.Appear(advice.id, {duration : 1.0 });	
				}*/
			}
			this[prop] = true;
...
/*Validation.add('validation-max-10-es','mas de diez',function(v){
 			return Validation.get('IsEmpty').test(v) || /^[a-zA-Z\W]{1,10}$/.test(v);
 	});*/
